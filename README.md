twitter クライアント sayaka ちゃん version 3.6.2 (2023/03/26)
======
...
* `--no-combine` … U+20DD〜U+20E4 の囲み文字の前にスペースを追加します。
	...
	手段として使われがちです。
	そのため、端末およびフォントによって合成表示した結果読めないものが
	表示されてしまう環境ではその肝心な部分だけが読めないという
	残念なことが起こります。それが場合によってはマシになるかも知れません。

	例えば "キートップの絵文字で囲んだ数字の 1" `1⃣` は
	`--no-combine` を指定すると `1 ⃣` のように表示されると思います。
	(ご使用の端末で cat ./README.md で表示させてみてください)
