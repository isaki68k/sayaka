twitter クライアント sayaka ちゃん version 3.5.5 (2023/02/28)
======

ターミナルに特化した twitter クライアントです。

* ~~フィルタストリームの垂れ流しが出来ます。~~
  (2023/03 twitter 側のサービスが停止したっぽいです)
...
キーワード(ハッシュタグなど)検索の場合はキーワードを1つ以上指定します。
複数指定した場合は OR になります。
```
% sayaka <keyword> [<keyword>...]
```
...
* `--reconnect <sec>` … ストリームのコネクションが切れた時の再接続までの
	時間を秒で指定します。0 を指定すると再接続を行いません。
	デフォルトは 5秒です。
