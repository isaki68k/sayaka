Misskey クライアント sayaka ちゃん version 3.8.0 (2024/09/xx)
======
...
* `--token <file>` … Misskey のアクセストークンが書かれたファイルを
	指定します。
