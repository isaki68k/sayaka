SIXEL 端末用画像ビューア sixelv version 3.8.6 (2026/01/28)
===
...
sixelv の変更点
---
* 3.8.7 (2026/05/30) … sixelv 側の変更はありません。
* 3.8.6 (2026/01/28) …
	BMP のデコードを stb_image から自前に変更してより多くの形式に対応。
	PNM (P1/P2/P3/P4) のデコードに対応。
	PNM (P5/P6) のデコードを stb_image から自前に変更。
	柳沢 PIC のデコードに対応。
	MAG のデコードに対応。
	ICO のデコードに対応。
	画像のロードを少し高速化。
...
* GIF
* ICO
* JPEG (ICC プロファイルには未対応)
* JPEG-XL (色空間は SRGB のみ)
* MAG
* PIC (柳沢 PIC)
* PNG
* PNM (P1〜P6 形式)
...
アニメーションには未対応ですが、各ページの静止画としての表示は可能です。
...
* `-b,--blurhash` … 入力を Blurhash 画像とみなします。
	Blurhash には自身を Blurhash であると識別するマジックなどがないためです。
...
* `-i` … 画像の情報を表示します。`--debug-image=1` と等価です。
* `--ipv4` / `--ipv6`
* `--ignore-error` … 画像が複数枚指定された場合に
	エラーが起きても次のファイルの処理に移ります。
* `--list-supported-images` …
	サポートしている画像形式とそのデコーダの一覧を表示します。
...
Blurhash について
---
Blurhash 画像は自身が Blurhash 画像であるという情報も持っていません。
そのため `--blurhash` オプションで明示する必要があります。
また、大きさもアスペクト比の情報も持っていません。
